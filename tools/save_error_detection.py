...
if __name__ == '__main__':
    ...
    filename = 'output/%s_false_positive.txt' % (classname)
    filename_rois = 'output/%s_false_positive_rois.txt' % (classname)
    f = open(filename, 'rb')
    d = pickle.load(f)
    f.close()
    ...
    for i in range(len(d)):
        ...
        plt.axis('off')
        plt.tight_layout()
        plt.savefig('/home/zhbli/temp/fp/%s_%s_%d' % (classname, d[i][0][0], i))
